...
while cv2.waitKey(2) == -1:
    ...
    # 左鍵鬆開，矩形畫好
        ...
        cv2.grabCut(img, mask, rect_copy, bgdModel, fgdModel, int(itr), cv2.GC_INIT_WITH_RECT)
        print(np.count_nonzero(mask))
        com_mask = np.where((mask == 2) | (mask == 0), 0, 1).astype('uint8')
        print(com_mask.shape)
        print(mask.shape)
        print(com_mask[:, :, np.newaxis].shape)
        output_img = img * com_mask[:, :, np.newaxis]
        # 顯示圖片分割後結果
        cv2.imshow('grabcut', output_img)
        # 顯示原圖
        cv2.imshow('img', img)
        cv2.imwrite(imgName+str(itr)+'itr_grabcut.jpg', output_img)
